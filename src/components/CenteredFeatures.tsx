...
const FeatureCard = ({ icon, title, description, svgComponent, color, delay, isActive, modalContent }: FeatureCardProps) => {
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div 
          className={`modern-feature-card ${isActive ? 'active' : ''} ${color}`}
        ...
        <div className="space-y-8 mt-6">
          ...
          {modalContent.security && (
            <div className="bg-gradient-to-r from-green-500/10 to-blue-500/10 p-6 rounded-2xl border border-green-500/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-lg font-semibold text-foreground">Almacenamiento Seguro y Privado</h3>
              </div>
              <p className="text-foreground leading-relaxed">{modalContent.security}</p>
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {modalContent.features.map((feature, index) => (
              <div key={index} className="p-6 bg-gradient-to-br from-white/5 to-white/2 rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-300">
...
- IP/Dispositivo: [Trazabilidad completa]`
  ...
  return (
    <div className={`centered-features-section ${isVisible ? 'visible' : ''}`}>
      {/* Enhanced Header */}
